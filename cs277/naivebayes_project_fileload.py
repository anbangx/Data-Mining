...
# Pre-Process Part
#
inputFile = open('pre_processed_data_object_naivebayes', 'rb')
...
for alpha in [0.08]:
    ...
    numberOfCorrectFiles = evaluateResult(fileAssignedCategoryTestNV, fileAssignedCategoryTest)
    percentTest = numberOfCorrectFiles/float(len(fileAssignedCategoryTest))
        
    endTimeTest = time.time()
    print " Execution Time (" + str(endTimeTest - startTimeTest) + " sec ) -  Testing the Data Set"
    print " Execution Time (" + str(endTimeTest - startTime) + " sec ) -  Overall (Training + Test)"
    print " Number of Correct Files " + str(numberOfCorrectFiles)
    print " Number of unique Files " + str(len(fileAssignedCategoryTest))
    print " Accuracy of test "  + str(percentTest)
