...
categoryNum = 0
categoryTestNum = 0
outputFile = open('pre_processed_data_object_decision_tree_' + dataSet + "_" + str(fileTestFractionSize), 'wb')
...
# tmpColumn = [0 for x in range(len(wholeVocabularyList)+1)]
#
...
#         # if len(tmpColumn) > 50:
#         #     break
#         tmpColumn1[idx] = value.get(term, 0)
#         idx += 1
#         # tmp = value.get(term)
#         # if tmp == None:
#         #     tmpColumn.append(0)
...
#
#     termFrequencyPerCategoryList.append(tmpColumn1)

print "Reading Test Set done. Elapsed Time:\t" + str(time.time() - startTime)
...
# Put frequency of each terms across entire test categories
...
thresholdForKeep = 0.00001
wholeVocabularySetNeedToRemove = set()
wholeVocabularyTestSetNeedToRemove = set()


for key in wholeVocabularySet:

    tmpFreq = float(wholeVocabularyFrequencyDict[key]) / float(totalFrequency)

    if tmpFreq < thresholdForKeep:
       wholeVocabularySetNeedToRemove.add(key)

wholeVocabularyList = list(wholeVocabularySet - wholeVocabularySetNeedToRemove)


for key in wholeTestVocabularySet:

    tmpFreq = float(wholeVocabularyTestFrequencyDict[key]) / float(totalFrequency)

    if tmpFreq < thresholdForKeep:
       wholeVocabularyTestSetNeedToRemove.add(key)

wholeTestVocabularyList = list(wholeTestVocabularySet - wholeVocabularyTestSetNeedToRemove)

print "Number of Unque Terms:" + str(len(wholeVocabularySet))
print "Number of Unique Terms after reduced:" + str(len(wholeVocabularyList))
print "Number of Unque Test Terms:" + str(len(wholeTestVocabularySet))
print "Number of Unque Test Terms after reduced:" + str(len(wholeTestVocabularyList))
...
#
#
# First, calculate term frequency per each word across whole file in the training set
tmpList = [0 for x in xrange(fileNum)]
...
for term in wholeVocabularyFromTrainingAndTestSetList:
    ...
    if term_count % 1000 == 0:
        print "[Training] Processed " + str(term_count) + "/" + str(len(wholeVocabularyFromTrainingAndTestSetList)) + "terms.\t" + str(time.time()-startTime)

    # key = filename, value : {category : {term:frequency ...}}
    for key, value in fileAlphaNumericStrStemmedDict.iteritems():

        tmpList2[tmpIdx] = value.itervalues().next().get(term,0)
        tmpIdx += 1

        # # key1 = category, value1 : {term:frequency... }
        # for key1, value1 in value.iteritems():
        #
        #     tmpList2[tmpIdx] = value1.get(term, 0)
        #     tmpIdx += 1
            # term is found
            # if term in value1:
            #     tmpList.append(value1[term])
...
print "[DT] frequencyInWordPerFileInTrainingSetList created. Elapsed Time:\t" + str(time.time()-startTime) + "\tSize:\t" + str(int(sys.getsizeof(frequencyInWordPerFileInTrainingSetList))/1048576) + "MB"

# Next, Reorganize the list per file, not per word
tmpList = [0 for x in xrange(len(frequencyInWordPerFileInTrainingSetList)+1)]
...
for key, value in fileAlphaNumericStrStemmedDict.iteritems():
    ...
    for i in range(0,len(frequencyInWordPerFileInTrainingSetList)):
        tmpList2[i] = frequencyInWordPerFileInTrainingSetList[i][tmpCount]
        
    for cat in fileBelongCategory[key]:
        tmpList2[-1] = cat
        frequencyInFilePerCategoryInTrainingSetList.append(tmpList2[:])
    
    tmpCount += 1

    #del tmpList2

print "[DT] frequencyInFilePerCategoryInTrainingSetList created. Elapsed Time:\t" + str(time.time()-startTime) + "\tSize:\t" + str(int(sys.getsizeof(frequencyInFilePerCategoryInTrainingSetList))/1048576) + "MB"
...
tmpList = [0 for x in xrange(fileTestNum)]
...
for term in wholeVocabularyFromTrainingAndTestSetList:
    ...
    if term_count % 1000 == 0:
        print "[Test] Processed " + str(term_count) + "/" + str(len(wholeVocabularyFromTrainingAndTestSetList)) + "terms. \t" + str(time.time() - startTime)
    ...
    # If max value is zero, then we don't need to apply sampling since all value is zero
    if maxValue == 0:
        frequencyInWordPerFileInTestSetList.append(tmpList2[:])
    else:
        interval = float((minValue + maxValue)) / numberOfBins
        bins = np.arange(minValue,maxValue,interval)
        for idx, val in enumerate(tmpList2):
...
print "[DT] frequencyInWordPerFileInTestSetList created. Elapsed Time:\t" + str(time.time()-startTime) + "\tSize:\t" + str(int(sys.getsizeof(frequencyInWordPerFileInTestSetList))/1048576) + "MB"

# Next, Reorganize the list per file, not per word
tmpList = [0 for x in xrange(len(frequencyInWordPerFileInTestSetList)+1)]
...
for key, value in fileTestAlphaNumericStrStemmedDict.iteritems():
    ...
    for i in range(0,len(frequencyInWordPerFileInTestSetList)):
        tmpList2[i] = frequencyInWordPerFileInTestSetList[i][tmpCount]
...
#         tmpList1.append(cat)
    ...
    frequencyInFilePerCategoryInTestSetList.append(tmpList2[:])
    
    tmpCount += 1
    #del tmpList2

print "[DT] frequencyInFilePerCategoryInTestSetList created. Elapsed Time:\t" + str(time.time()-startTime) + "\tSize:\t" + str(int(sys.getsizeof(frequencyInFilePerCategoryInTestSetList))/1048576) + "MB"
...
print "Done. Elapsed Time:\t" + str(time.time() - startTime)
